...
async function remoteFetch(url: string, cookies: any[], headers = {}) {
...
const queueLoop = new PQueue({ concurrency: 1 })
let cookie = ''
let useCount = 0
let validMatchine = {
  state: false,
...
function createWork() {
  return new Promise(async (resolve) => {
    ...
    if (useCount >= 60 || !validMatchine.state) {
      useCount = 0
      ...
      const { status, message } = await remoteFetch('https://www.bing.com/turing/captcha/challenge', requestCookies, initHeaders)
      debug('captcha done')
...
async function createContext() {
  ...
  console.log('add queue', id)
  emitter.emit('create', id)
  const { cookie, json, headers } = await pEvent(emitter, id, { timeout: 59000 }) || {} as any
  if (
    !json
  ) {
  ...
  return { cookie, json, headers }
}

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    debug('new')
    const { json, cookie, headers } = await createContext()
    const bingCookie = btoa(`curl -H 'cookie: ${cookie}'`)
    res.setHeader('set-cookie', [
      ...[`BING_HEADER=${bingCookie.trim()}`].map(c => `${c}; Max-Age=${86400 * 30}; Path=/;`),
      ...[`BING_IP=`].map(c => `${c}; Max-Age=0; Path=/;`)
    ])
